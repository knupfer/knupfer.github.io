...
FUN.modelliteration = function(fun.fit.formula, fun.pcitems, fun.iterator, fun.data)
{
    fun.tempfit = NULL
    if (fun.iterator %in% fun.pcitems)
    {
        fun.tempfit[[fun.iterator]] = polr(fun.fit.formula, data = fun.data)
    } else
    {
        fun.tempfit[[fun.iterator]] = glm(fun.fit.formula, data = fun.data, family = "binomial")
    }
    return(fun.tempfit)
}
...
for (i in 1:length(test)) 
{
    ...
    fit = FUN.modelliteration(fit.formula, pcitems, i, test)
}
...
for (k in 1:persons)
{
    ...
    for (i in 2:items)
    {
            ...
            while (upper >= lower)
            {
                ...
                for (u in 1:length(query))
                {
                    ...
                    } else if (u == length(query))
                    {
                        ...
                        {
                            found = database[current, (u+3)]
                            rest.temp[i-1] = database[current, (u+2)]
                            query = c(query, found)
                            break
                        } else
                        {
        ...
        ## search in new data if binary search is'nt succesfull
        if (!found && exists("newdata"))
        {
            for (m in 1:length(newdata[, 1]))
            {
                for (u in 1:length(query))
                {
                    ...
                    {
                        found = newdata[m, (length(query)+2)]
                        rest.temp[i-1] = newdata[m, (length(query)+1)]
                        query = c(query, found)
                    }
                }
        ...
        if (!found)
        {
            ...
            for (q in 1:length(test[-query]))
            {
                ...
                fit = FUN.modelliteration(fit.formula, pcitems.temp, q, test)
            }
            ...
            Liste = foreach(j=1:length(test[-query])) %dopar%
            {
                ...
                if (length(test[-query]) > 1 && isgood[j])
                {
                    ...
                    for (n in 1:length(test[-query][-j]))
                    {
                        ...
                        fit2 = FUN.modelliteration(fit2.formula, pcitems.temp2, n, test)
                    }
    ...
    } else
    {
        ...
        for (j in 1:length(test[-query]))
        {
            ...
            fit = FUN.modelliteration(fit.formula, pcitems.temp, j, test)
        }
