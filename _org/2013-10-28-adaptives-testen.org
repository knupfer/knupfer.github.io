...
### Calculate the potential benefit of an adaptive test with non-adaptive data (data.dat).
#
# Return a database (database.dat) of proposed items for each individual.
# Draw a curve of the effect.
...
FUN.modelliteration = function(fun.fit.formula, fun.pcitems)
{
...
# Return the modell for the first item. (This should be a function.)
for (i in 1:length(test)) 
{
    fit.formula = reformulate('1', names(test[i]))
    fit[[i]] = FUN.modelliteration(fit.formula, pcitems)
}
...
for (k in 1:persons)
{
    ...
    for (i in 2:items)
    {
        ...
        if (!found)
        {
            ...
            for (q in 1:length(test[-query]))
            {
                ...
                fit[[q]] = FUN.modelliteration(fit.formula, pcitems.temp)
            }
            
            # Calculate the entropie of these items and chose good ones.
            info.temp = FUN.EntroMC(pcitems.temp, test[k, ], fit)
            isgood = info.temp >= (max(info.temp)[1] * 0.8)
            ...
            Liste = foreach(j=1:length(test[-query])) %dopar%
            {
                ...
                if (length(test[-query]) > 1 && isgood[j])
                {
                    ...
                    for (n in 1:length(test[-query][-j]))
                    {
                        ...
                        fit2[[n]] = FUN.modelliteration(fit2.formula, pcitems.temp2)
                    }
    ...
    } else
    {
        ...
        for (j in 1:length(test[-query]))
        {
            ...
            fit[[j]] = FUN.modelliteration(fit.formula, pcitems.temp)
        }
