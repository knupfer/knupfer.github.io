...
#+NAME: statistic
#+BEGIN_SRC R :session muu :exports code :results output :noweb yes
require(MASS)
library(foreach)
library(doMC)
...
for (k in 1:persons)
{
    ...
    for (i in 2:items)
    {
        ...
        if (found == 0 && exists("newdata"))
        {
            for (m in 1:length(newdata[,1]))
            {
                for (u in 1:length(query))
                {
                    ...
                    if (newdata[m,(length(query)+3)] == 0 && u == length(query))
                    {
        ...
        if (found == 0)
        {
            ...
            for (q in 1:length(test[-query]))
            {
                ...
                if (q %in% pcitems.temp)
                {
                    fit[[q]] = polr(fit.formula, data = test)
                } else
                {
                    fit[[q]] = glm(fit.formula, data = test, family = "binomial"(link=logit))   
                }
            }
            ...
            Liste = foreach(j=1:length(test[-query])) %dopar%
            {
                ...
                if (length(test[-query]) > 1 && isgood[j])
                {
                    ...
                    for (n in 1:length(test[-query][-j]))
                    {
                        ...
                        if (n %in% pcitems.temp2)
                        {
                            fit2[[n]] = polr(fit2.formula, data = test)
                        } else
                        { 
                            fit2[[n]] = glm(fit2.formula, data = test, family = "binomial"(link=logit))
                        }
                    }
    ...
    } else
    {
        ...
        for (j in 1:length(test[-query]))
        {
            ...
            if (j %in% pcitems.temp)
            {
                fit[[j]] = polr(fit.formula, data = test)
            } else
            {
                fit[[j]] = glm(fit.formula, data = test, family = "binomial"(link=logit))
            }
        }
